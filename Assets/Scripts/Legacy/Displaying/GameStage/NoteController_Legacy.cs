...
public class NoteController_Legacy : MonoBehaviour
{
    public int id;
    private float time;
    ...
    private void PositionUpdate()
    {
        ...
        if (stage.editor.noteSelect[id].prevSelected != stage.editor.noteSelect[id].selected) //Note selected
        {
            ...
            noteSprite.color = new Color(85.0f / 255, 192.0f / 255, 1.0f, alpha); // 蓝色
        }
        else if (stage.collided[id] && !curNote.isLink)
        {
            alpha = noteSprite.color.a;
            noteSprite.color = new Color(1.0f, 85.0f / 255, 85.0f / 255, alpha); // 红色
        }
        if (z <= 0.0f)
        {
            z = 0.0f;
