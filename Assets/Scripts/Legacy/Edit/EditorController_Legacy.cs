...
public class EditorController_Legacy : MonoBehaviour
{
    //-Editor activation-
    ...
    public LocalizedText_Legacy amountSel;
    ...
    public LocalizedText_Legacy isLinkSel;
    public LocalizedText_Legacy pianoSoundsSel;
    ...
    private List<NoteIndicatorController_Legacy> noteIndicators = new List<NoteIndicatorController_Legacy>();
    ...
    public void UpdateNoteIndicators()
    {
        ...
        else
        {
            ...
            NoteIndicatorController_Legacy indicator;
        ...
        if (Utility.GetMouseWorldPos().y < -1.0f || Input.mousePosition.x > Utility.stageWidth)
            foreach (NoteIndicatorController_Legacy indicator in noteIndicators) indicator.NoColor();
        else
        {
            Vector2 pos = SetDragPosition(new Vector2(0.0f, 0.0f));
            if (snapToGrid) pos = QuantizePosition(pos);
            foreach (NoteIndicatorController_Legacy indicator in noteIndicators)
            {
                float offset = pos.x;
                if (pasteMode)
    ...
    {
        NoteIndicatorController_Legacy indicator = noteIndicators[id];
        indicator.gameObject.SetActive(true);
        indicator.gameObject.transform.SetParent(noteIndicatorParent);
