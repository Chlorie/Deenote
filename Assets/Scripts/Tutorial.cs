...
"
        ...
        new List<string> // Language 1 (Chinese)
        {
            ...
            "4.1.3 谱面编辑\n" +
            "按住鼠标左键并拖动可以框选音符，按住Ctrl框选可以多选；单击鼠标右键可以在鼠标所在位置放置音符。\n\n",
            "4.2 编辑板块\n" +
            ...
            "这一部分显示被选中的音符的基本信息，分别是：Position，音符横向位置，显示在界面上的音符横向位置都在-2到2之间，包含两端，另外有只播放钢琴音" +
            "而游玩时不显示的音符，这些音符的横向位置超出这个范围，目前本软件暂不支持这种类型音符的编辑，不过可能会在以后的更新中添加对这些音符的支持；" +
            "Time，时间，即为音符落到判定线上的时间；Size，音符大小，一般大小为1，本程序设定最大为5最小为0.1，不过为了观赏/游玩体验，建议最大不要超过2" +
            "最小不要小于0.5；Shift，这个值所有人都不知道是干什么用的所以还是不要动了吧；Is link note，若选中的音符是滑键则此项为True，不是滑键则" +
            "为False；Piano sounds，钢琴音符附带的钢琴音信息。其中，若对于某一项选中的音符有不同的多个值，则该项显示为Several Values；若没有选择" +
            "任何音符，则所有项显示为Not Available。这些项中，音符横向位置、时间、音符大小、Shift值和钢琴音可以点击修改，若要对钢琴音进行修改，点击" +
            "[Click to view]则会打开钢琴界面。\n" +
            "4.2.2.1 钢琴界面\n" +
            "在选中音符信息板块中点击钢琴音信息编辑的[Click to view]字样会打开钢琴界面。该界面出现在屏幕的下方，分为左右两个部分。左部显示当前编辑的" +
            "音符的钢琴音信息，右边的是128键钢琴窗口。点击左面的[Save]按钮可以将现在的钢琴信息保存到已经选中的音符中并关闭钢琴窗，[Play]按钮可以试听" +
            "目前的钢琴音，[Back]按钮直接关闭钢琴窗而不保存目前的钢琴音信息。左面下方为各个钢琴音信息的显示区，每一个钢琴音有四个属性，分别是Duration，" +
            "音长；Delay，延迟，即从音符判定到该声音响起经过的延迟时间；Pitch，音调，该钢琴音的音调；Volume，该钢琴音的音量（强度），最大值是127。" +
            "除了音调以外的三个值都可以点击修改，并且点击右下角的[Delete]按钮可以删除这一条钢琴音。点击右面钢琴的琴键可以添加一个钢琴音，音调为对应" +
            "琴键的音调，而剩下的三个值默认都是0。\n\n" +
            "4.2.3 播放预览设置(Player Settings)\n" +
            "这一板块包含谱面预览/音乐播放的基本设置。分别为：Note speed，音符下落速度，与游戏中一样为0.5到9.5的半整数，可以通过左右两个箭头按钮调节，" +
            ...
            "4.2.4 音符摆放与格线(Note Placement & Grid)\n" +
            "该板块包含有关音符摆放和格线显示的设置。分别为：Vertical grid，竖直格线数量，以中线（0横向位置）为对称轴在横向区域上均匀地画出线，下方的" +
            "Offset（偏移）即为对称轴的偏移量，可调范围为-1到1，可以通过直接输入数字和滑动滑杆两种方式进行调节（例：早期官方谱面中音符的横向位置大多" +
            "落在-2，-1.75，-1.5……1.75，2这九条线上，在本程序中若想让竖直格线在这些线上，可以设置竖直格线数量为8，偏移量为0.25）；Horizontal grid" +
            "横向格线数量，设置后会将每一拍平均分成对应数目份（比如需要16分音符的格线，这项应设置为4，因为一般情况下16分音符是一拍的四分之一，同理需要" +
            "16分三连音/俗称24分音符格线时，这项要设置为6）；Snap to grid，对齐格线，此项开启时放置音符会自动吸附到最近的格线上，可以使用快捷键G来" +
            "切换这一项的状态；Show note indicator，显示音符指示，开启此项后，鼠标在谱面编辑区中的时候会显示淡化的音符来指示音符会摆放到的位置；Show" +
            " border，显示边界，开启此项后谱面编辑区域中会显示两条粗线表示-2和2的横向边界。\n\n" +
            "4.2.5 曲线生成(Curve Forming)\n" +
            "该板块包含生成曲线以及快速批量填充音符功能。点击[Cubic]按钮（或快捷键I）可以作出过选定音符的自然三次样条插值曲线，点击[Linear]按钮（或快" +
            "捷键Shift+I）可以作出过选定音符的线性插值曲线，在生成曲线以后，除首尾以外的中间所有音符会被自动删除。通俗点讲，就是Cubic生成的是一条过指" +
            "定音符的光滑曲线，而Linear生成的是通过这些音符的折线段。Fill amount为填充数量，[Fill Notes]按钮（或快捷键F）在已生成的曲线上均匀放置" +
            "上面设置的填充数量个音符。在曲线开启时，对齐到格线功能会把音符对齐到曲线而不是格线。点击[Disable]或使用快捷键Ctrl+I关闭曲线。\n\n" +
            ...
            "，在选中音符的时候，这些值会被自动改成Start为选中的第一个音符的时间，End为选中的最后一个音符的时间，BPM为使一拍长度为Start到End时间差的值" +
            ...
            "BPM为180的拍线，再在四拍后的位置放置一个音符，选中这个音符，从这个音符的时间开始以BPM170填充拍线到乐曲末尾，即可完成拍线的填充）。\n\n\n",
            "5. 设置板块\n" +
