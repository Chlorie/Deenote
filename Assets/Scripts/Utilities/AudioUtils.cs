...
namespace Deenote.Utilities
{
    ...
    {
        public static async UniTask<MayBeNull<AudioClip>> LoadAsync(Stream stream, string audioType, CancellationToken cancellationToken = default)
        {
            var result = await UniTask.RunOnThreadPool(Load, configureAwait: true, cancellationToken: cancellationToken);

            if (result is not (var data, var reader))
                return null;
