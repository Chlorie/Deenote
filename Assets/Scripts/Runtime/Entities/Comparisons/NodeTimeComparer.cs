...
{
    public sealed class NodeTimeComparer : IComparer<IStageTimeNode>, IComparer<SpeedLineValueModel>
    {
        public static NodeTimeComparer Instance { get; } = new();

        public int Compare(IStageTimeNode x, IStageTimeNode y)
            => Comparer<float>.Default.Compare(x.Time, y.Time);
        public int Compare(SpeedLineValueModel x, SpeedLineValueModel y)
