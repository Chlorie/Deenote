...
namespace Deenote.Entities.Models
{
    public static partial class ModelOperations
    {
        ...
        private static ImmutableArray<T> CtorGetValues<T>(ReadOnlySpan<NoteModel> notes, Func<NoteModel, T> getter)
        {
            var values = new T[notes.Length];
            for (int i = 0; i < notes.Length; i++)
                values[i] = getter.Invoke(notes[i]);
